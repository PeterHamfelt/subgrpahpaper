""" 
Loading the dataset, converting it into a graph and extracting node features
Training a Graph Convolutional Network (GCN) model to obtain node embeddings
Clustering the nodes based on their embeddings
Identifying subgraphs corresponding to each cluster
Detecting anomalies based on node attributes and graph structure
Combining attribute-based and structure-based anomaly scores
Identifying the anomalies in the graph
Here's a step-by-step explanation of the methodology:

Load the dataset: The script starts by loading the Cora dataset using PyTorch Geometric's Planetoid function. The graph data is then converted into a NetworkX graph, and node features are added as attributes.

Train GCN: A Graph Convolutional Network (GCN) is defined and trained on the dataset to obtain node embeddings. The training process includes early stopping, which prevents overfitting.

Extract embeddings: Once the model is trained, the node embeddings are extracted and normalized.

Clustering: The nodes are clustered using K-means with the initial centroids calculated by the K-means++ algorithm. The clustering is then evaluated using various clustering evaluation metrics.

Subgraph extraction: The script identifies subgraphs for each cluster in the graph.

Threshold calculation: Thresholds for node attributes and graph structure are calculated based on a 15% criterion. This includes the number of nodes, average shortest path length, and graph density.

Anomaly detection: Anomalies are detected based on the node attributes and graph structure, separately. Each subgraph is compared to the threshold values, and the anomaly score is calculated.

Combine anomaly scores: The attribute-based and structure-based anomaly scores are combined by applying weights to each score (40% for attribute-based scores and 60% for structure-based scores) and summing them up.

Identify anomalies: Anomalies are identified based on the weighted combined anomaly scores. Any cluster with a score greater than 0 is considered anomalous.
