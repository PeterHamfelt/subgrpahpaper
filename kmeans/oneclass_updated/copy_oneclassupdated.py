...
# Get node embeddings from the trained model
with torch.no_grad():
    log_probabilities = model(data.x, data.edge_index)

# Exponentiate log probabilities to get probabilities
probabilities = torch.exp(log_probabilities)
anomaly_probabilities = probabilities[:, 0].numpy()  # Assuming 0 is the anomaly class
true_labels = (labels == 0).numpy().astype(int)
auc_roc = roc_auc_score(true_labels, anomaly_probabilities)
